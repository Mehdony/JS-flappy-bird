    ...
    if (gamePlaying) {
        pipes.map(pipe => {
            ...
            ctx.drawImage(img, 432 + pipeWidth, 108, pipeWidth, canvas.height - pipe[1]
                + pipeGap, pipe[0], pipe[1] + pipeGap, pipeWidth, canvas.height - pipe[1] + pipeGap)

            if (pipe[0] <= -pipeWidth) {
                currentScore++,
                    bestScore = Math.max(bestScore, currentScore);

                // Faire disparaitre les poteaux puis les recréers

                pipes = [...pipes.slice(1), [pipes[pipes.length - 1][0] + pipeGap + pipeWidth,
                pipeLoc()]];
            }
            // Si tu tapes le poteaux, fin de partie
